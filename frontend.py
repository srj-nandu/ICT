...
def register_routes(app: Any, handlers: dict[str, Any]) -> None:
    ...
    def dashboard() -> Any:
        ...
        if not user:
            return handlers["redirect_with_params"](LOGIN_PAGE, {"message": "Please log in to open the dashboard.", "kind": "warning"})
        return handlers["build_dashboard_page"](user, request.args.get("message", ""), request.args.get("kind", "success"))

    @app.route(UPLOAD_PAGE, methods=["GET"])
    def upload_page() -> Any:
        user = handlers["get_current_user"]()
        if not user:
            return handlers["redirect_with_params"](LOGIN_PAGE, {"message": "Please log in to upload resumes.", "kind": "warning"})
        return handlers["build_upload_page"](user, request.args.get("message", ""), request.args.get("kind", "success"))

    @app.route(PROFILE_PAGE, methods=["GET"])
    def profile_page() -> Any:
        user = handlers["get_current_user"]()
        if not user:
            return handlers["redirect_with_params"](LOGIN_PAGE, {"message": "Please log in to open your profile.", "kind": "warning"})
        return handlers["build_profile_page"](user, request.args.get("message", ""), request.args.get("kind", "success"))

    @app.route(f"{RESULT_PAGE}/<result_id>", methods=["GET"])
    def result_page(result_id: str) -> Any:
        user = handlers["get_current_user"]()
        if not user:
            return handlers["redirect_with_params"](LOGIN_PAGE, {"message": "Please log in to view analysis results.", "kind": "warning"})
        try:
            return handlers["build_result_page"](user, result_id, request.args.get("message", ""), request.args.get("kind", "success"))
    ...
    def contact() -> Any:
        ...
        return handlers["build_contact_page"](user, request.args.get("message", ""), request.args.get("kind", "success"), form_data)

    @app.route(LOGIN_PAGE, methods=["GET"])
    def login_page() -> Any:
        if handlers["get_current_user"]():
            return redirect(DASHBOARD_PAGE)
        return handlers["build_login_page"](request.args.get("message", ""), request.args.get("kind", "success"), request.args.get("email", ""))
    ...
    def login_post() -> Any:
        try:
            user = handlers["authenticate_user"](request.form.get("email", ""), request.form.get("password", ""))
            handlers["store_current_user"](user)
            return handlers["redirect_with_params"](DASHBOARD_PAGE, {"message": f"Welcome back, {user['name']}.", "kind": "success"})
        except ValueError as exc:
    ...
    def logout_post() -> Any:
        ...
        return handlers["redirect_with_params"](LOGIN_PAGE, {"message": "You have been logged out.", "kind": "success"})
    ...
    def contact_post() -> Any:
        try:
            ...
            return handlers["redirect_with_params"](CONTACT_PAGE, {"message": "Your message has been saved. We will reach out soon.", "kind": "success"})
        except ValueError as exc:
    ...
    def profile_post() -> Any:
        user = handlers["get_current_user"]()
        if not user:
            return handlers["redirect_with_params"](LOGIN_PAGE, {"message": "Please log in to update your profile.", "kind": "warning"})
        try:
            ...
            handlers["store_current_user"](user)
            return handlers["redirect_with_params"](PROFILE_PAGE, {"message": "Profile updated successfully.", "kind": "success"})
        except ValueError as exc:
    ...
    def resume_delete_post() -> Any:
        user = handlers["get_current_user"]()
        if not user:
            return handlers["redirect_with_params"](LOGIN_PAGE, {"message": "Please log in to delete resumes.", "kind": "warning"})
        try:
            ...
            return handlers["redirect_with_params"](PROFILE_PAGE, {"message": "Resume and generated analysis files deleted.", "kind": "success"})
        except ValueError as exc:
            return handlers["redirect_with_params"](PROFILE_PAGE, {"message": str(exc), "kind": "warning"})

    @app.route("/old-data-delete", methods=["POST"])
    def old_data_delete_post() -> Any:
        user = handlers["get_current_user"]()
        if not user:
            return handlers["redirect_with_params"](LOGIN_PAGE, {"message": "Please log in to delete old data.", "kind": "warning"})
        ...
        return handlers["redirect_with_params"](return_to, {"message": f"Deleted {deleted_count} old data {label} and generated files.", "kind": "success"})

    @app.route("/delete", methods=["POST"])
    def delete_post() -> Any:
        if not handlers["get_current_user"]():
            return handlers["redirect_with_params"](LOGIN_PAGE, {"message": "Please log in to manage records.", "kind": "warning"})
        try:
            record_id = request.form.get("record_id", "").strip()
            if not record_id:
                raise ValueError("No record id was provided for deletion.")
            handlers["delete_record"](record_id)
            return handlers["redirect_with_params"](DASHBOARD_PAGE, {"message": "Record deleted and its folder was removed from storage.", "kind": "success"})
        except ValueError as exc:
    ...
    def bulk_delete_post() -> Any:
        if not handlers["get_current_user"]():
            return handlers["redirect_with_params"](LOGIN_PAGE, {"message": "Please log in to manage records.", "kind": "warning"})
        record_ids = [record_id.strip() for record_id in request.form.getlist("record_ids") if record_id.strip()]
        if not record_ids:
            return handlers["redirect_with_params"](DASHBOARD_PAGE, {"message": "Select at least one record first.", "kind": "warning"})
        ...
        return handlers["redirect_with_params"](DASHBOARD_PAGE, {"message": f"Deleted {deleted_count} selected {label}.", "kind": "success"})
